...
const S = {
  ...
  card: (isFirst) => ({ background: '#fff', borderRadius: 18, marginBottom: 10, overflow: 'hidden', border: isFirst ? '1.5px solid #c9a84c' : '1.5px solid #e8e0d0', boxShadow: isFirst ? '0 4px 20px rgba(201,168,76,0.15)' : 'none' }),
    ...
    <div style={S.page}>
      <div style={S.wrap}>
        <div style={S.header}>
          ...
          {lastUpdated && <div style={S.updated}>Scores updated {new Date(lastUpdated).toLocaleTimeString()}</div>}
          {locked && <div style={S.lockedPill}>🔒 Picks Locked</div>}
        </div>
        ...
        <div style={S.body}>
          ...
          {ranked.length === 0 ? (
            ...
            ranked.map((entry, i) => (
              <div key={entry.nickname} style={S.card(i===0)}>
                <button style={S.cardBtn} onClick={() => setExpanded(expanded === entry.nickname ? null : entry.nickname)}>
                  <div style={S.rank}>
                    {i < 3 ? medals[i] : <span style={S.rankNum}>{i+1}</span>}
                  </div>
                  <div style={S.nameWrap}>
                    <div style={S.name}>{entry.nickname}</div>
                    <div>{entry.paid === false
                  ...
                  <span style={S.chevron}>{expanded === entry.nickname ? '▲' : '▼'}</span>
                </button>
